...
extension NSToolbarItem.Identifier {
    public static let toggleButton = NSToolbarItem.Identifier("toggleButton")
}
...
class SettingsWindow: NSWindow, NSWindowDelegate, NSToolbarDelegate {
    ...
    private let sidebarView: SidebarView = SidebarView(
        frame: NSRect(x: 0, y: 0, width: 180, height: 480))

    private var dashboard: NSView = Dashboard()
    private var settings: ApplicationSettings = ApplicationSettings()
    private var servers: ServersSettings = ServersSettings()
    ...
    init() {
        ...
        let newToolbar = NSToolbar(identifier: "com.4ving.Mornits.Settings.Toolbar")
        ...
        NSLayoutConstraint.activate([
            ...
            self.mainView.container.topAnchor.constraint(
                equalTo: (self.contentLayoutGuide as! NSLayoutGuide).topAnchor),
            self.mainView.container.bottomAnchor.constraint(
                equalTo: (self.contentLayoutGuide as! NSLayoutGuide).bottomAnchor),
        ])

        NotificationCenter.default.addObserver(
            self, selector: #selector(menuCallback), name: .openModuleSettings, object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(toggleSettingsHandler), name: .toggleSettings, object: nil)
        NotificationCenter.default.addObserver(
    ...
    @objc private func menuCallback(_ notification: Notification) {
        if let title = notification.userInfo?["module"] as? String {
            ...
            if ["GPU", "Sensors", "Battery", "Bluetooth", "Clock"].contains(title) {
                self.title += " (\(localizedString("Local Only")))"
            }
    ...
    @objc private func toggleEnable(_ sender: NSControl) {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: ["module": moduleName, "state": controlState(sender)])
    }
    ...
    internal func setModules() {
        ...
        if !self.pauseState
            && modules.filter({
                $0.enabled != false && $0.available != false
                    && !$0.menuBar.widgets.filter({ $0.isActive }).isEmpty
            }).isEmpty
        {
            self.setIsVisible(true)
        }
...
private class MainView: NSView {
    ...
    override init(frame: NSRect) {
        ...
        let foreground = NSVisualEffectView(
            frame: NSRect(x: 0, y: 0, width: frame.width, height: frame.height))
...
private class SidebarView: NSStackView {
    ...
    private var dashboardIcon: NSImage {
        if #available(macOS 11.0, *),
            let icon = NSImage(
                systemSymbolName: "circle.grid.3x3.fill", accessibilityDescription: nil)
        {
            return icon
        }
    ...
    private var serverIcon: NSImage {
        if #available(macOS 11.0, *), let icon = iconFromSymbol(name: "server.rack", scale: .large)
        {
            return icon
        }
        return NSImage(named: NSImage.Name("server"))!
    }

    override init(frame: NSRect) {
        self.scrollView = ScrollableStackView(
            frame: NSRect(x: 0, y: 0, width: frame.width, height: frame.height))
        ...
        self.scrollView.stackView.addArrangedSubview(
            MenuItem(icon: self.dashboardIcon, title: "Dashboard"))
        self.scrollView.stackView.addArrangedSubview(
            MenuItem(icon: self.serverIcon, title: "Servers"))
        ...
        let additionalButtons: NSStackView = NSStackView(
            frame: NSRect(x: 0, y: 0, width: frame.width, height: 45))
        ...
        let pauseButton = self.makeButton(
            title: localizedString("Pause Mornits"),
            image: self.pauseState ? self.resumeIcon : self.pauseIcon,
            action: #selector(togglePause))
        self.pauseButton = pauseButton

        additionalButtons.addArrangedSubview(
            self.makeButton(
                title: localizedString("Settings"), image: self.settingsIcon,
                action: #selector(openSettings)))
        additionalButtons.addArrangedSubview(
            self.makeButton(
                title: localizedString("Support the application"), image: self.supportIcon,
                action: #selector(donate)))
        additionalButtons.addArrangedSubview(
            self.makeButton(
                title: localizedString("Report a bug"), image: self.bugIcon,
                action: #selector(reportBug)))
        additionalButtons.addArrangedSubview(pauseButton)
        additionalButtons.addArrangedSubview(
            self.makeButton(
                title: localizedString("Close application"), image: self.closeIcon,
        ...
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenForPause), name: .pause, object: nil)
    }
    ...
    fileprivate func setModules(_ list: [Module]) {
        let order: [String] = [
            "CPU", "RAM", "Disk", "Network", "GPU", "Sensors", "Battery", "Bluetooth", "Clock",
        ]
        let sortedList = list.sorted {
            let index1 = order.firstIndex(of: $0.config.name) ?? Int.max
            let index2 = order.firstIndex(of: $1.config.name) ?? Int.max
            return index1 < index2
        }

        sortedList.reversed().forEach { (m: Module) in
    ...
    private func supportView() -> NSViewController {
        ...
        let github = SupportButtonView(
            name: "GitHub Sponsors", image: "github",
            action: {
                NSWorkspace.shared.open(URL(string: "https://github.com/sponsors/4ving")!)
            })
        let paypal = SupportButtonView(
            name: "PayPal", image: "paypal",
            action: {
                NSWorkspace.shared.open(
                    URL(string: "https://paypal.me/4ving/4USD")!)
            })
        let koFi = SupportButtonView(
            name: "Ko-fi", image: "ko-fi",
            action: {
                NSWorkspace.shared.open(URL(string: "https://ko-fi.com/4ving")!)
            })
    ...
    @objc private func openSettings() {
        NotificationCenter.default.post(
            name: .openModuleSettings, object: nil, userInfo: ["module": "Settings"])
    }

    @objc private func reportBug() {
        NSWorkspace.shared.open(
            URL(string: "https://github.com/4ving/Mornits/issues/new?template=bug_report.md")!)
    }

    @objc private func donate(_ sender: NSButton) {
        self.supportPopover.show(
            relativeTo: sender.bounds, of: sender, preferredEdge: NSRectEdge.minY)
    }
    ...
    @objc private func togglePause() {
        ...
        self.pauseButton?.toolTip = localizedString(
            self.pauseState ? "Resume Mornits" : "Pause Mornits")
        self.pauseButton?.image = self.pauseState ? self.resumeIcon : self.pauseIcon
        NotificationCenter.default.post(
            name: .pause, object: nil, userInfo: ["state": self.pauseState])
    }
...
private class MenuItem: NSView {
    ...
    init(icon: NSImage?, title: String) {
        ...
        let titleView = TextView(
            frame: NSRect(x: 34, y: ((32 - 16) / 2) + 1, width: 100, height: 16))
    ...
    fileprivate func activate() {
        ...
        NotificationCenter.default.post(
            name: .openModuleSettings, object: nil, userInfo: ["module": self.title])
