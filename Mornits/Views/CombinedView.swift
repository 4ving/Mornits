...
internal class CombinedView: NSObject, NSGestureRecognizerDelegate {
    ...
    private var view: NSView = NSView(
        frame: NSRect(x: 0, y: 0, width: 0, height: Constants.Widget.height))
    ...
    override init() {
        ...
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenForOneView), name: .toggleOneView, object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenForModuleRearrrange), name: .moduleRearrange,
            object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenCombinedModulesPopup), name: .combinedModulesPopup,
            object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenForModule), name: .toggleModule, object: nil)
    }
        ...
        self.activeModules.forEach { (m: Module) in
            ...
            if self.separator && i < 2 * self.activeModules.count - 1 {
                let separator = NSView(
                    frame: NSRect(x: w, y: 3, width: 1, height: Constants.Widget.height - 6))
                separator.wantsLayer = true
                separator.layer?.backgroundColor =
                    (separator.isDarkMode ? NSColor.black : NSColor.white).cgColor
    ...
    @objc private func togglePopup(_ sender: NSButton) {
        guard let popup = self.popup, let item = self.menuBarItem, let window = item.button?.window
        else { return }
    ...
    @objc private func listenForModule(_ notification: Notification) {
        guard let name = notification.userInfo?["module"] as? String,
            ...
            state,
            let module = self.activeModules.first(where: { $0.name == name })
        else { return }
...
private class Popup: NSStackView, Popup_p {
    ...
    init() {
        self.keyboardShortcut =
            Store.shared.array(key: "CombinedModules_popup_keyboardShortcut", defaultValue: [])
            as? [UInt16] ?? []
        ...
        NotificationCenter.default.addObserver(
            self, selector: #selector(reinit), name: .toggleModule, object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(reinit), name: .moduleRearrange, object: nil)
    }
    ...
    @objc private func reinit() {
        ...
        let availableModules = modules.filter({ $0.enabled && $0.portal != nil }).sorted(by: {
            $0.combinedPosition < $1.combinedPosition
        })
