...
internal class SetupWindow: NSWindow, NSWindowDelegate {
    ...
    init() {
        ...
        self.delegate = self
        self.title = localizedString("Mornits Setup")
...
private class SetupView_1: NSStackView {
    init() {
        ...
        title.stringValue = localizedString("Welcome to Mornits")
        title.toolTip = localizedString("Welcome to Mornits")
...
private class SetupView_end: NSStackView {
    init() {
        ...
        let paypal = SupportButtonView(name: "PayPal", image: "paypal", action: {
            NSWorkspace.shared.open(URL(string: "https://paypal.me/4ving/4USD")!)
        })
