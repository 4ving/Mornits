...
let updater = Updater(github: "4ving/Mornits", url: "https://api.github.com/repos/4ving/Mornits/releases/latest")
var modules: [Module] = [
    CPU(),
    GPU(),
    RAM(),
...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    internal let updateActivity = NSBackgroundActivityScheduler(identifier: "com.4ving.Mornits.updateCheck")
    internal let supportActivity = NSBackgroundActivityScheduler(identifier: "com.4ving.Mornits.support")
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        info("Mornits started in \((startingPoint.timeIntervalSinceNow * -1).rounded(toPlaces: 4)) seconds")
        self.startTS = Date()
    }
            ...
            if let url = URL(string: uri) {
                updater.download(url, completion: { path in
                    updater.install(path: path) { error in
                        if let error {
                            showAlert("Error update Mornits", error, .critical)
                        }
                    }
                })
            }
