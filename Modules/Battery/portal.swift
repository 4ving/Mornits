...
internal class Portal: NSStackView, Portal_p {
    ...
    public init(_ module: ModuleType) {
        ...
        box.edgeInsets = NSEdgeInsets(
            top: 0, left: Constants.Popup.spacing * 2, bottom: 0, right: Constants.Popup.spacing * 2
        )
    ...
    public func loadCallback(_ value: Battery_Usage) {
        DispatchQueue.main.async(execute: {
            ...
            if value.timeToEmpty != -1 && value.timeToEmpty != 0 {
                seconds = Double(
                    (value.isBatteryPowered ? value.timeToEmpty : value.timeToCharge) * 60)
            }
            self.timeField.stringValue =
                seconds != 0
                ? seconds.printSecondsToHoursMinutesSeconds(short: self.timeFormat == "short") : ""

            self.batteryView.setValue(abs(value.level))

            if !value.isBatteryPowered {
                self.chargingField.stringValue =
                    value.isCharging ? localizedString("Charging") : localizedString("Connected")
