...
public struct GPU_entry: TimelineEntry {
    ...
    public static var snapshot: GPU_entry = GPU_entry(
        value: GPU_Info(
            id: "", type: "", IOClass: "", model: "", cores: nil, utilization: 0.11, render: 0.11,
            tiler: 0.11))
            ...
            VStack(spacing: 10) {
                if let value = entry.value {
                    HStack {
                        Chart {
                            SectorMark(
                                angle: .value(localizedString("Used"), value.utilization ?? 0),
                                innerRadius: .ratio(0.8)
                            ).foregroundStyle(self.usedColor)
                            SectorMark(
                                angle: .value(
                                    localizedString("Free"), 1 - (value.utilization ?? 0)),
                                innerRadius: .ratio(0.8)
                    ...
                    VStack(spacing: 3) {
                        HStack {
                            Text(localizedString("Usage")).font(.system(size: 12, weight: .regular))
                                .foregroundColor(.secondary)
                            Spacer()
                            Text("\(Int((value.utilization ?? 0)*100))%")
                        }
                        HStack {
                            Text(localizedString("Render")).font(
                        ...
                        HStack {
                            Text(localizedString("Tiler")).font(.system(size: 12, weight: .regular))
                                .foregroundColor(.secondary)
                            Spacer()
