...
internal class Popup: PopupWrapper {
    ...
    private var readColor: NSColor {
        self.readColorState.additional as? NSColor ?? NSColor.systemRed
    }
    private var writeColorState: SColor = .secondRed
    private var writeColor: NSColor {
        self.writeColorState.additional as? NSColor ?? NSColor.systemBlue
    }
    ...
    public init(_ module: ModuleType) {
        ...
        self.readColorState = SColor.fromString(
            Store.shared.string(
                key: "\(self.title)_readColor", defaultValue: self.readColorState.key))
        self.writeColorState = SColor.fromString(
            Store.shared.string(
                key: "\(self.title)_writeColor", defaultValue: self.writeColorState.key))
    ...
    private func recalculateHeight() {
        var h: CGFloat = 0
        h +=
            self.disks.subviews.map({ $0.frame.height + self.disks.spacing }).reduce(0, +)
            - self.disks.spacing
    ...
    private func initProcesses() -> NSView {
        ...
        let view: NSView = NSView(
            frame: NSRect(x: 0, y: 0, width: self.frame.width, height: self.processesHeight))
        ...
        let container: ProcessesView = ProcessesView(
            ...
            values: [
                (localizedString("Read"), self.readColor),
                (localizedString("Write"), self.writeColor),
            ],
    ...
    internal func capacityCallback(_ value: Disks) {
        defer {
            let h =
                self.disks.subviews.map({ $0.bounds.height + self.disks.spacing }).reduce(0, +)
                - self.disks.spacing
        ...
        self.disks.subviews.filter { $0 is DiskView }.map { $0 as! DiskView }.forEach {
            (v: DiskView) in
            if !value.array.map({ $0.uuid }).contains(v.uuid) {
                v.removeFromSuperview()
            }
        }
        value.forEach { (drive: drive) in
            if let view = self.disks.subviews.filter({ $0 is DiskView }).map({ $0 as! DiskView })
                .first(where: { $0.uuid == drive.uuid })
            {
                view.update(free: drive.free, smart: drive.smart)
            } else {
                self.disks.addArrangedSubview(
                    DiskView(
    ...
    internal func activityCallback(_ value: Disks) {
        ...
        value.reversed().forEach { (drive: drive) in
            if let view = views.first(where: { $0.uuid == drive.uuid }) {
...
internal class DiskView: NSStackView {
    ...
    init(
        width: CGFloat, uuid: String = "", name: String = "", size: Int64 = 1, free: Int64 = 1,
        path: URL? = nil, smart: smart_t? = nil, resize: @escaping () -> Void
    ) {
        ...
        self.legendView = LegendView(
            width: innerWidth, id: "\(name)_\(path?.absoluteString ?? "")", size: size, free: free)
        self.detailsView = DetailsView(
            width: innerWidth, id: "\(name)_\(path?.absoluteString ?? "")", smart: smart)
    ...
    public func setChartReverseOrder(_ newValue: Bool) {
        ...
        self.detailsView.setReverseOrder(newValue)
    }
...
internal class NameView: NSStackView {
    ...
    private var readColor: NSColor {
        SColor.fromString(
            Store.shared.string(
                key: "\(ModuleType.disk.stringValue)_readColor", defaultValue: SColor.secondBlue.key
            )
    ...
    private var writeColor: NSColor {
        SColor.fromString(
            Store.shared.string(
                key: "\(ModuleType.disk.stringValue)_writeColor", defaultValue: SColor.secondRed.key
            )
        ).additional as! NSColor
    }
    ...
    public func update(free: Int64?, read: Int64?, write: Int64?) {
        if (self.window?.isVisible ?? false) || !self.ready {
            if let read = read {
                ...
                self.readState?.layer?.backgroundColor =
                    read != 0
                    ? self.readColor.cgColor : NSColor.lightGray.withAlphaComponent(0.75).cgColor
            }
            if let write = write {
                self.writeState?.toolTip = "Write: \(Units(bytes: write).getReadableSpeed())"
                self.writeState?.layer?.backgroundColor =
                    write != 0
                    ? self.writeColor.cgColor : NSColor.lightGray.withAlphaComponent(0.75).cgColor
            }
            self.ready = true
        }
    }
    ...
    @objc private func openDisk() {
        if let uri = self.uri, let finder = self.finder {
            NSWorkspace.shared.open(
                [uri], withApplicationAt: finder, configuration: NSWorkspace.OpenConfiguration())
        }
    }
...
internal class ChartView: NSStackView {
    ...
    public init(width: CGFloat) {
        ...
        let chart = NetworkChartView(
            ...
            ), num: 120, reversedOrder: self.reverseOrder, outColor: self.writeColor,
            inColor: self.readColor)
    ...
    override func updateLayer() {
        self.layer?.backgroundColor =
            self.isDarkMode
            ? NSColor.lightGray.withAlphaComponent(0.1).cgColor : NSColor.white.cgColor
    }
...
internal class BarView: NSView {
    ...
    public init(width: CGFloat, size: Int64, free: Int64) {
        ...
        let view: NSView = NSView(
            frame: NSRect(x: 1, y: 0, width: self.frame.width - 2, height: self.frame.height))
        ...
        let percentage = size > 0 ? CGFloat(size - free) / CGFloat(size) : 0
        let width: CGFloat = (view.frame.width * (percentage < 0 ? 0 : percentage)) / 1
        self.usedBarSpace = NSView(
            frame: NSRect(x: 0, y: 0, width: width, height: view.frame.height))
    ...
    override func updateLayer() {
        self.background?.layer?.backgroundColor =
            self.isDarkMode
            ? NSColor.lightGray.withAlphaComponent(0.1).cgColor : NSColor.white.cgColor
    }
    ...
    public func update(free: Int64?) {
        if (self.window?.isVisible ?? false) || !self.ready {
            if let free = free, self.usedBarSpace != nil {
                let percentage = self.size > 0 ? CGFloat(self.size - free) / CGFloat(self.size) : 0
                let width: CGFloat =
                    ((self.frame.width - 2) * (percentage < 0 ? 0 : percentage)) / 1
                self.usedBarSpace?.setFrameSize(
                    NSSize(width: width, height: self.usedBarSpace!.frame.height))
            }
...
internal class LegendView: NSView {
    ...
    public init(width: CGFloat, id: String, size: Int64, free: Int64) {
        ...
        let view: NSView = NSView(
            frame: NSRect(x: 0, y: 0, width: self.frame.width, height: self.frame.height))

        let legendField = TextView(
            frame: NSRect(
                x: 0, y: (view.frame.height - height) / 2, width: view.frame.width - 40,
                height: height))
        ...
        let percentageField = TextView(
            frame: NSRect(
                x: view.frame.width - 40, y: (view.frame.height - height) / 2, width: 40,
                height: height))
    ...
    private func legend(free: Int64) -> String {
        ...
        if self.showUsedSpace {
            ...
            value = localizedString(
                "Used disk memory", DiskSize(usedSpace).getReadableMemory(),
                DiskSize(self.size).getReadableMemory())
        } else {
            value = localizedString(
                "Free disk memory", DiskSize(free).getReadableMemory(),
                DiskSize(self.size).getReadableMemory())
        }
...
internal class DetailsView: NSStackView {
    ...
    private var readRowView: NSView?
    private var writeRowView: NSView?
    private var speedContainer: NSStackView?
    ...
    private func initSpeed() -> NSView {
        ...
        let container: NSStackView = NSStackView(
            frame: NSRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height))
        container.orientation = .vertical
        container.spacing = 0
        self.speedContainer = container

        var row: (NSView, LabelField, ValueField)

        row = popupWithColorRow(
            container, color: self.readColor, title: "\(localizedString("Read")):", value: "0 KB/s")
        self.readSpeedValueField = row.2
        self.readColorView = row.0
        self.readRowView = container.arrangedSubviews.last

        row = popupWithColorRow(
            container, color: self.writeColor, title: "\(localizedString("Write")):",
            value: "0 KB/s")
        self.writeSpeedValueField = row.2
        self.writeColorView = row.0
        self.writeRowView = container.arrangedSubviews.last

        if self.reverseOrder {
            // If reversed, Write should be first
            // But popUpWithColorRow adds to container automatically.
            // We need to re-arrange.
            self.setReverseOrder(true)
        }

        self.totalReadValueField =
            popupRow(container, title: "\(localizedString("Total read")):", value: "0 KB").1
        self.totalWrittenValueField =
            popupRow(container, title: "\(localizedString("Total written")):", value: "0 KB").1
    ...
    private func initSmart() -> NSView {
        let view: NSView = NSView(
            frame: NSRect(x: 0, y: 0, width: self.frame.width, height: self.smartHeight))
        view.widthAnchor.constraint(equalToConstant: view.bounds.width).isActive = true
        view.heightAnchor.constraint(equalToConstant: view.bounds.height).isActive = true
        let separator = separatorView(
            localizedString("SMART"),
            origin: NSPoint(x: 0, y: self.smartHeight - Constants.Popup.separatorHeight),
            width: self.frame.width)
        let container: NSStackView = NSStackView(
        ...
        self.smartTotalReadValueField =
            popupRow(container, title: "\(localizedString("Total read")):", value: "0 KB").1
        self.smartTotalWrittenValueField =
            popupRow(container, title: "\(localizedString("Total written")):", value: "0 KB").1
        self.temperatureValueField =
            popupRow(
                container, title: "\(localizedString("Temperature")):", value: "\(temperature(0))"
            ).1
        self.healthValueField =
            popupRow(container, title: "\(localizedString("Health")):", value: "0%").1
        self.powerCyclesValueField =
            popupRow(container, title: "\(localizedString("Power cycles")):", value: "0").1
        self.powerOnHoursValueField =
            popupRow(container, title: "\(localizedString("Power on hours")):", value: "0").1
    ...
    private var readColorView: NSView?
    private var writeColorView: NSView?
    ...
    public func update(stats: stats) {
        ...
        self.totalWrittenValueField?.stringValue = Units(bytes: stats.writeBytes)
            .getReadableMemory()
    ...
    private func updateColors() {
        let readColor =
            SColor.fromString(
                Store.shared.string(
                    key: "\(ModuleType.disk.stringValue)_readColor",
                    defaultValue: SColor.secondBlue.key)
            ).additional as! NSColor
        let writeColor =
            SColor.fromString(
                Store.shared.string(
                    key: "\(ModuleType.disk.stringValue)_writeColor",
                    defaultValue: SColor.secondRed.key)
            ).additional as! NSColor

        if self.readColorView?.layer?.backgroundColor != readColor.cgColor {
            self.readColorView?.layer?.backgroundColor = readColor.cgColor
        }
        if self.writeColorView?.layer?.backgroundColor != writeColor.cgColor {
            self.writeColorView?.layer?.backgroundColor = writeColor.cgColor
        }
    }
    ...
    public func update(smart: smart_t?) {
        ...
        self.smartTotalReadValueField?.stringValue = Units(bytes: smart.totalRead)
            .getReadableMemory()
        self.smartTotalWrittenValueField?.stringValue = Units(bytes: smart.totalWritten)
            .getReadableMemory()
    ...
    public func setReverseOrder(_ val: Bool) {
        guard let container = self.speedContainer,
            let readRow = self.readRowView,
            let writeRow = self.writeRowView
        else { return }

        if container.arrangedSubviews.contains(readRow) {
            container.removeArrangedSubview(readRow)
        }
        if container.arrangedSubviews.contains(writeRow) {
            container.removeArrangedSubview(writeRow)
        }

        if val {
            container.insertArrangedSubview(writeRow, at: 0)
            container.insertArrangedSubview(readRow, at: 0)
        } else {
            container.insertArrangedSubview(readRow, at: 0)
            container.insertArrangedSubview(writeRow, at: 0)
        }
