...
internal class DiskSettings: NSStackView, Settings_v, NSTextFieldDelegate {
    ...
    private var disksContainer: NSStackView = NSStackView()
    ...
    public init(_ module: ModuleType) {
        ...
        self.removableState = Store.shared.bool(
            key: "\(self.title)_removable", defaultValue: self.removableState)
        self.updateIntervalValue = Store.shared.int(
            key: "\(self.title)_updateInterval", defaultValue: self.updateIntervalValue)
        ...
        self.SMARTState = Store.shared.bool(
            key: "\(self.title)_SMART", defaultValue: self.SMARTState)
    ...
    public func load(widgets: [widget_t]) {
        ...
        // Container for dynamic disks list
        self.disksContainer.orientation = .vertical
        self.disksContainer.spacing = Constants.Settings.margin
        self.addArrangedSubview(self.disksContainer)

        self.addArrangedSubview(
            PreferencesSection([
                PreferencesRow(
                    localizedString("Show removable disks"),
    ...
    internal func setList(_ list: Disks) {
        var newIDs: [String] = []
        list.forEach { d in
            newIDs.append(d.isNetwork ? d.uuid : d.mediaName)
        }

        if self.diskIDs == newIDs && !self.disksContainer.subviews.isEmpty { return }
        self.diskIDs = newIDs

        DispatchQueue.main.async(execute: {
            self.disksContainer.subviews.forEach { $0.removeFromSuperview() }

            self.disksContainer.isHidden = list.isEmpty
            if list.isEmpty { return }

            let section = PreferencesSection(label: localizedString("Disk"))

            list.forEach { d in
                let key = d.isNetwork ? d.uuid : d.mediaName
                let state = Store.shared.bool(
                    key: "Disk_state_\(key)", defaultValue: d.isNetwork ? true : d.root)
                let btn = self.switchView(action: #selector(self.toggleDisk), state: state)
                btn.identifier = NSUserInterfaceItemIdentifier(key)

                var label = d.mediaName
                if !d.isNetwork {
                    label = "\(localizedString("Local")): \(d.mediaName)"
                }

                section.add(PreferencesRow(label, component: btn))
            }

            self.disksContainer.addArrangedSubview(section)
        })
    }

    // Cache for identifying if list changed
    private var diskIDs: [String] = []

    @objc private func toggleDisk(_ sender: NSControl) {
        guard let id = sender.identifier else { return }
        Store.shared.set(key: "Disk_state_\(id.rawValue)", value: controlState(sender))
        self.callback()
    }
