...
public struct Disk_entry: TimelineEntry {
    ...
    public static var snapshot: Disk_entry = Disk_entry(
        value: drive(size: 494_384_795_648, free: 251_460_125_440))
                ...
                if let value = entry.value {
                    HStack {
                        Chart {
                            SectorMark(
                                angle: .value(
                                    localizedString("Used"),
                                    (100 * (value.size - value.free)) / value.size),
                                innerRadius: .ratio(0.8)
                            ).foregroundStyle(self.usedColor)
                            SectorMark(
                                angle: .value(
                                    localizedString("Free"), (100 * value.free) / value.size),
                                innerRadius: .ratio(0.8)
                            ).foregroundStyle(self.freeColor)
                        }
                    ...
                    VStack(spacing: 3) {
                        HStack {
                            Rectangle().fill(self.usedColor).frame(width: 12, height: 12)
                                .cornerRadius(2)
                            Text(localizedString("Used")).font(.system(size: 12, weight: .regular))
                                .foregroundColor(.secondary)
                            Spacer()
                            Text(DiskSize(value.size - value.free).getReadableMemory())
                        }
                        HStack {
                            Rectangle().fill(self.freeColor).frame(width: 12, height: 12)
                                .cornerRadius(2)
                            Text(localizedString("Free")).font(.system(size: 12, weight: .regular))
                                .foregroundColor(.secondary)
                            Spacer()
