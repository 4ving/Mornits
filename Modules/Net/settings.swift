...
internal class NetSettings: NSStackView, Settings_v, NSTextFieldDelegate {
    public var numberOfProcesses: Int = 8
    ...
    private var generalSection: PreferencesSection? = nil
    ...
    private let interfacesContainer: NSStackView = {
    ...
    public init(_ module: ModuleType) {
        ...
        self.numberOfProcesses = Store.shared.int(
            key: "\(self.title)_processes", defaultValue: self.numberOfProcesses)
        self.usageReset = Store.shared.string(
            key: "\(self.title)_usageReset", defaultValue: self.usageReset)
        self.VPNModeState = Store.shared.bool(
            key: "\(self.title)_VPNMode", defaultValue: self.VPNModeState)
        self.widgetActivationThresholdState = Store.shared.bool(
            key: "\(self.title)_widgetActivationThresholdState",
            defaultValue: self.widgetActivationThresholdState)
        self.widgetActivationThreshold = Store.shared.int(
            key: "\(self.title)_widgetActivationThreshold",
            defaultValue: self.widgetActivationThreshold)
        self.widgetActivationThresholdSize = SizeUnit.fromString(
            Store.shared.string(
                key: "\(self.title)_widgetActivationThresholdSize",
                defaultValue: self.widgetActivationThresholdSize.key))
        self.ICMPHost = Store.shared.string(
            key: "\(self.title)_ICMPHost", defaultValue: self.ICMPHost)
        self.updateICMPIntervalValue = Store.shared.int(
            key: "\(self.title)_updateICMPInterval", defaultValue: self.updateICMPIntervalValue)
        self.publicIPState = Store.shared.bool(
            key: "\(self.title)_publicIP", defaultValue: self.publicIPState)
        self.publicIPRefreshInterval = Store.shared.string(
            key: "\(self.title)_publicIPRefreshInterval", defaultValue: self.publicIPRefreshInterval
        )
        self.baseValue = Store.shared.string(
            key: "\(self.title)_base", defaultValue: self.baseValue)
        self.textValue = Store.shared.string(
            key: "\(self.title)_textWidgetValue", defaultValue: self.textValue)
        ...
        for interface in SCNetworkInterfaceCopyAll() as NSArray {
            if let bsdName = SCNetworkInterfaceGetBSDName(interface as! SCNetworkInterface),
                let displayRef = SCNetworkInterfaceGetLocalizedDisplayName(
                    interface as! SCNetworkInterface)
            {
                ...
                self.list.append(
                    Network_interface(
                        displayName: displayName as String, BSDName: name))
            }
        }
        self.setupLocalObserver()
    }
    ...
    private func setupLocalObserver() {
        NotificationCenter.default.addObserver(
            self, selector: #selector(localEnabledDidChange),
            name: UserDefaults.didChangeNotification, object: nil)
    }

    @objc private func localEnabledDidChange(_ notification: Notification) {
    ...
    public func setList(_ list: [Network_interface]) {
        DispatchQueue.main.async {
            if self.list.count == list.count
                && self.list.map({ $0.BSDName }) == list.map({ $0.BSDName })
                && !self.interfacesContainer.subviews.isEmpty
            {
                return
            }
            self.list = list
    ...
    private func renderList() {
        DispatchQueue.main.async(execute: {
            self.interfacesContainer.subviews.forEach { $0.removeFromSuperview() }

            let section = PreferencesSection(label: localizedString("Network interface"))

            self.list.forEach { interface in
                // Hide local interface if disabled in main settings
                if !interface.BSDName.contains(":") && !RemoteServersManager.shared.localEnabled {
                    return
                }

                let state = Store.shared.bool(
                    key: "Network_state_\(interface.BSDName)", defaultValue: true)
                let btn = self.switchView(action: #selector(self.toggleInterface), state: state)
                btn.identifier = NSUserInterfaceItemIdentifier(interface.BSDName)
                section.add(PreferencesRow(interface.displayName, component: btn))
            }

            self.interfacesContainer.addArrangedSubview(section)
        })
    }

    @objc private func toggleInterface(_ sender: NSControl) {
        guard let id = sender.identifier else { return }
        Store.shared.set(key: "Network_state_\(id.rawValue)", value: controlState(sender))
        self.callback()
    }
    ...
    public func load(widgets: [widget_t]) {
        ...
        self.section = PreferencesSection([
            PreferencesRow(
                localizedString("Number of top processes"),
        ...
        self.addArrangedSubview(self.interfacesContainer)
        // Initial set if list is not empty (local only initially)
        if !self.list.isEmpty {
            self.setList(self.list)
        }
        self.interfacesContainer.isHidden = false
        ...
        var prefs: [PreferencesRow] = [
            ...
            PreferencesRow(
                localizedString("Reset data usage"),
                component: selectView(
            ...
            PreferencesRow(
                localizedString("Auto-refresh public IP address"),
                component: selectView(
        ...
        if self.vpnConnection {
            prefs.append(
                PreferencesRow(
                    localizedString("VPN mode"),
                    component: switchView(
                        action: #selector(self.toggleVPNMode),
                        state: self.VPNModeState
                    )))
        }
        let generalSection = PreferencesSection(prefs)
        generalSection.setRowVisibility(5, newState: self.publicIPState)
        self.addArrangedSubview(generalSection)
        self.generalSection = generalSection

        self.widgetThresholdSection = PreferencesSection([
            PreferencesRow(
                localizedString("Widget activation threshold"),
                component: PreferencesSwitch(
                    ...
                    with: StepperInput(
                        ...
                        callback: self.changeWidgetActivationThreshold,
                        unitCallback: self.toggleWidgetActivationThresholdSize
                    )
                ))
        ])
        self.addArrangedSubview(self.widgetThresholdSection!)
        self.widgetThresholdSection?.setRowVisibility(
            1, newState: self.widgetActivationThresholdState)
        ...
        let ICMPField = self.inputField(
            id: "ICMP", value: self.ICMPHost,
        ...
        self.addArrangedSubview(
            PreferencesSection([
                ...
                PreferencesRow(
                    localizedString("Update interval"),
                    component: selectView(
        ...
        if widgets.contains(where: { $0 == .text }) {
            let textField = self.inputField(
                id: "text", value: self.textValue,
                placeholder: localizedString("This will be visible in the text widget"))
            self.addArrangedSubview(
                PreferencesSection([
                    PreferencesRow(localizedString("Text widget value"), component: textField) {
                        [weak self] in
    ...
    private func toggleWidgetActivationThresholdSize(_ newValue: KeyValue_p) {
        ...
        Store.shared.set(
            key: "\(self.title)_widgetActivationThresholdSize",
            value: self.widgetActivationThresholdSize.key)
        self.display()
    }
    ...
    @objc private func toggleRefreshIPInterval(_ sender: NSMenuItem) {
        ...
        Store.shared.set(
            key: "\(self.title)_publicIPRefreshInterval", value: self.publicIPRefreshInterval)
