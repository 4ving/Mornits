...
public class Portal: PortalWrapper {
    ...
    private var base: DataSizeBase {
        DataSizeBase(rawValue: Store.shared.string(key: "\(self.name)_base", defaultValue: "byte"))
            ?? .byte
    }
    ...
    private var chartScale: Scale {
        Scale.fromString(
            Store.shared.string(key: "\(self.name)_chartScale", defaultValue: Scale.none.key))
    }
    ...
    private var chartFixedScaleSize: SizeUnit {
        SizeUnit.fromString(
            Store.shared.string(
                key: "\(self.name)_chartFixedScaleSize", defaultValue: SizeUnit.MB.key))
    }

    private var downloadColor: NSColor {
        let v = SColor.fromString(
            Store.shared.string(
                key: "\(self.name)_downloadColor", defaultValue: SColor.secondBlue.key))
    ...
    private var uploadColor: NSColor {
        let v = SColor.fromString(
            Store.shared.string(key: "\(self.name)_uploadColor", defaultValue: SColor.secondRed.key)
        )
    ...
    public override func load() {
        ...
        // Main container box (was Chart container)
        // Adjust height if needed? The original chart height was 68.
        // The container frame width was self.frame.width - (Constants.Popup.spacing*8).
        // Let's keep the chart logic as is.

        let container: NSView = NSView(
            frame: CGRect(
                x: 0, y: 0, width: self.frame.width - (Constants.Popup.spacing * 8), height: 68))
    ...
    public func usageCallback(_ value: Network_Usage) {
        DispatchQueue.main.async(execute: {
            if let chart = self.chart {
                ...
                chart.addValue(
                    upload: Double(value.bandwidth.upload),
                    download: Double(value.bandwidth.download))
                chart.setScale(
                    self.chartScale, Double(self.chartFixedScaleSize.toBytes(self.chartFixedScale)))
