...
public struct BLEDevice: Codable {
    ...
    var id: String {
        self.uuid?.uuidString ?? self.address
    }

    var state: Bool {
        Store.shared.bool(key: "ble_\(self.id)", defaultValue: false)
    }
    ...
    init(
        address: String, name: String, uuid: UUID?, RSSI: Int?, batteryLevel: [KeyValue_t],
        isConnected: Bool, isPaired: Bool
    ) {
...
public class Bluetooth: Module {
    ...
    private let portalView: Portal = Portal(.bluetooth)
    private let settingsView: BluetoothSettings = BluetoothSettings()
    ...
    private func batteryCallback(_ raw: [BLEDevice]?) {
        ...
        DispatchQueue.main.async(execute: {
            ...
            self.portalView.callback(active)
