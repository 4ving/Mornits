...
public class CPU: Module {
    ...
    private let settingsView: CPUSettings
    ...
    private var systemColor: NSColor {
        ...
        let key = Store.shared.string(
            key: "\(self.config.name)_systemColor", defaultValue: color.key)
    ...
    private var eCoreColor: NSColor {
        ...
        let key = Store.shared.string(
            key: "\(self.config.name)_eCoresColor", defaultValue: color.key)
        if let c = SColor.fromString(key).additional as? NSColor {
            return c
        }
        return color.additional as! NSColor
    ...
    private var pCoreColor: NSColor {
        ...
        let key = Store.shared.string(
            key: "\(self.config.name)_pCoresColor", defaultValue: color.key)
        if let c = SColor.fromString(key).additional as? NSColor {
            return c
        }
        return color.additional as! NSColor
    ...
    public init() {
        self.settingsView = CPUSettings(.CPU)
    ...
    @objc private func remoteUpdate() {
        let aggregated = self.aggregatedLoad()
        self.portalView.callback(aggregated)
        self.updateMenuBar(aggregated)
    }

    private var lastLocalLoad: CPU_Load? = nil

    private func aggregatedLoad() -> CPU_Load {
        var totalSystem: Double = 0
        var totalUser: Double = 0
        var count: Double = 0

        // Start with empty response. Only use local data as base if enabled.
        var response = CPU_Load()

        if RemoteServersManager.shared.localEnabled {
            if let local = self.lastLocalLoad {
                response = local
                totalSystem += local.systemLoad
                totalUser += local.userLoad
                count += 1
            }
        }
        ...
        for server in servers {
            if let data = RemoteServersManager.shared.data[server.id] {
                if let details = data.cpuDetails {
                    totalSystem += details.system
                    totalUser += details.user
                    count += 1
                } else if let cpu = data.cpu {
                    totalUser += cpu
                    count += 1
                }
            }
        ...
        if count > 0 {
            response.systemLoad = totalSystem / count
            response.userLoad = totalUser / count
            response.totalUsage = response.systemLoad + response.userLoad
            response.idleLoad = 1.0 - response.totalUsage
        }
    ...
    private func loadCallback(_ raw: CPU_Load?) {
        ...
        if self.systemWidgetsUpdatesState {
            if #available(macOS 11.0, *) {
                if isWidgetActive(self.userDefaults, [CPU_entry.kind, "UnitedWidget"]),
    ...
    private func updateMenuBar(_ value: CPU_Load) {
        self.menuBar.widgets.filter { $0.isActive }.forEach { [self] (w: SWidget) in
            ...
            case let widget as BarChart:
                    ...
                    if widget.colorState == .cluster {
                        ...
                        for (i, v) in value.usagePerCore.enumerated() {
                            ...
                            val.append([
                                ColorValue(
                                    v,
                                    color: core?.type == .efficiency
                                        ? self.eCoreColor : self.pCoreColor)
                            ])
                        }
                    } else {
                ...
                } else if self.groupByClustersState, let e = value.usageECores,
                    let p = value.usagePCores
                {
                    if widget.colorState == .cluster {
                        val = [
