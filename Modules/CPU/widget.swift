...
public struct CPU_entry: TimelineEntry {
    ...
    public static var snapshot: CPU_entry = CPU_entry(
        value: CPU_Load(totalUsage: 0.34, systemLoad: 0.11, userLoad: 0.23, idleLoad: 0.66))
...
public struct Provider: TimelineProvider {
    ...
    public func getTimeline(
        in context: Context, completion: @escaping (Timeline<CPU_entry>) -> Void
    ) {
        var entry = CPU_entry()
        var loaded = false
            ...
            forSecurityApplicationGroupIdentifier: "N7LBX474DC.group.com.4ving.Mornits.shared")
        {
            ...
            if let data = try? Data(contentsOf: plistURL) {
                if let dict = try? PropertyListSerialization.propertyList(from: data, options: [], format: nil) as? [String: Any] {
                    if let rawData = dict["CPU@LoadReader"] as? Data {
                         if let load = try? JSONDecoder().decode(CPU_Load.self, from: rawData) {
                            entry.value = load
                            loaded = true
                         }
                    }
        ...
        if !loaded {
            if let raw = userDefaults?.data(forKey: "CPU@LoadReader"),
                let load = try? JSONDecoder().decode(CPU_Load.self, from: raw)
            {
                entry.value = load
            }
        }
            ...
            VStack(spacing: 10) {
                if let value = entry.value {
                    HStack {
                        Chart {
                            SectorMark(
                                angle: .value(localizedString("System"), value.systemLoad),
                                innerRadius: .ratio(0.8)
                            ).foregroundStyle(self.systemColor)
                            SectorMark(
                                angle: .value(localizedString("User"), value.userLoad),
                                innerRadius: .ratio(0.8)
                            ).foregroundStyle(self.userColor)
                            SectorMark(
                                angle: .value(localizedString("Idle"), value.idleLoad),
                                innerRadius: .ratio(0.8)
                            ).foregroundStyle(self.idleColor)
                        }
                    ...
                    VStack(spacing: 3) {
                        HStack {
                            Rectangle().fill(self.systemColor).frame(width: 12, height: 12)
                                .cornerRadius(2)
                            Text(localizedString("System")).font(
                        ...
                        HStack {
                            Rectangle().fill(self.userColor).frame(width: 12, height: 12)
                                .cornerRadius(2)
                            Text(localizedString("User")).font(.system(size: 12, weight: .regular))
                                .foregroundColor(.secondary)
                            Spacer()
