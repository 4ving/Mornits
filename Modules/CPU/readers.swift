...
internal class LoadReader: Reader<CPU_Load> {
    ...
    public override func setup() {
        self.hasHyperthreadingCores =
            sysctlByName("hw.physicalcpu") != sysctlByName("hw.logicalcpu")
        [CTL_HW, HW_NCPU].withUnsafeBufferPointer { mib in
            var sizeOfNumCPUs: size_t = MemoryLayout<uint>.size
            let status = sysctl(
                processor_info_array_t(mutating: mib.baseAddress), 2, &numCPUs, &sizeOfNumCPUs, nil,
                0)
            if status != 0 {
                self.numCPUs = 1
            }
    ...
    public override func read() {
        // Safe access to shared settings
        ...
        if localEnabled {
            let result: kern_return_t = host_processor_info(
                mach_host_self(), PROCESSOR_CPU_LOAD_INFO, &self.numCPUsU, &self.cpuInfo,
                &self.numCpuInfo)
            if result == KERN_SUCCESS {
                ...
                for i in 0..<Int32(numCPUs) {
                    ...
                    if let prevCpuInfo = self.prevCpuInfo {
                        inUse =
                            self.cpuInfo[Int(CPU_STATE_MAX * i + CPU_STATE_USER)]
                ...
                let showHyperthratedCores = Store.shared.bool(
                    key: "CPU_hyperhreading", defaultValue: false)
                if showHyperthratedCores || !self.hasHyperthreadingCores {
                    ...
                    while i < Int(self.usagePerCore.count / 2) {
                        a = i * 2
                        if self.usagePerCore.indices.contains(a)
                            && self.usagePerCore.indices.contains(a + 1)
                        {
                            self.response.usagePerCore.append(
                                (Double(self.usagePerCore[a]) + Double(self.usagePerCore[a + 1]))
                                    / 2)
                        }
                        i += 1
                    }
                ...
                if let prevCpuInfo = self.prevCpuInfo {
                    let prevCpuInfoSize: size_t =
                        MemoryLayout<integer_t>.stride * Int(self.numPrevCpuInfo)
                    vm_deallocate(
                ...
                error(
                    "host_processor_info(): \(String(cString: mach_error_string(result), encoding: String.Encoding.ascii) ?? "unknown error")",
                    log: self.log)
            }
        } else {
            // Clear per-core usage if local is disabled
            self.response.usagePerCore = []
        }

        // Calculate Local System/User Load
        var localSystem: Double = 0
        var localUser: Double = 0
        var localIdle: Double = 0

        let cpuInfo = hostCPULoadInfo()
        if let info = cpuInfo {
            let userDiff = Double(info.cpu_ticks.0 - self.previousInfo.cpu_ticks.0)
            let sysDiff = Double(info.cpu_ticks.1 - self.previousInfo.cpu_ticks.1)
            let idleDiff = Double(info.cpu_ticks.2 - self.previousInfo.cpu_ticks.2)
            let niceDiff = Double(info.cpu_ticks.3 - self.previousInfo.cpu_ticks.3)
            let totalTicks = sysDiff + userDiff + niceDiff + idleDiff

            if totalTicks > 0 {
                localSystem = sysDiff / totalTicks
                localUser = userDiff / totalTicks
                localIdle = idleDiff / totalTicks
            }
        ...
        if localEnabled {
            totalSystem += localSystem
            totalUser += localUser
            count += 1

            // Populate response with local data
            self.response.systemLoad = localSystem
            self.response.userLoad = localUser
            self.response.idleLoad = localIdle
            self.response.totalUsage = localSystem + localUser
        } else {
            self.response.systemLoad = 0
            self.response.userLoad = 0
            self.response.idleLoad = 0
            self.response.totalUsage = 0
        }

        // Remote aggregation removed. Now handled in main.swift.

        // Per-core E/P (Local only)
        // If local is disabled, these should probably be nil or 0
        if localEnabled, let cores = self.cores {
            let eCoresList: [Double] = cores.filter({ $0.type == .efficiency }).enumerated()
                .compactMap { (i, c) -> Double? in
            ...
            let pCoresList: [Double] = cores.filter({ $0.type == .performance }).enumerated()
                .compactMap { (i, c) -> Double? in
                    if self.response.usagePerCore.indices.contains(Int(c.id)) {
                        return self.response.usagePerCore[Int(c.id)]
                    }
            ...
            self.response.usageECores = nil
            self.response.usagePCores = nil
        }
...
public class TemperatureReader: Reader<Double> {
    ...
    public override func setup() {
        ...
        case .m2, .m2Pro, .m2Max, .m2Ultra:
            self.list = [
                "Tp1h", "Tp1t", "Tp1p", "Tp1l", "Tp01", "Tp05", "Tp09", "Tp0D", "Tp0X", "Tp0b",
                "Tp0f", "Tp0j",
            ]
        ...
        case .m4, .m4Pro, .m4Max, .m4Ultra:
            self.list = [
                "Te05", "Te09", "Te0H", "Te0S", "Tp01", "Tp05", "Tp09", "Tp0D", "Tp0V", "Tp0Y",
                "Tp0b", "Tp0e",
            ]
        default: break
        }
...
public class FrequencyReader: Reader<CPU_Frequency> {
    ...
    public override func read() {
        guard !self.isReading, !self.eCoreFreqs.isEmpty && !self.pCoreFreqs.isEmpty,
            self.channels != nil, self.subscription != nil
        else { return }
        self.isReading = true
        ...
        Task {
            ...
            for (samples, _) in await self.getSamples() {
                ...
                for sample in samples {
                    ...
                    if sample.channel.starts(with: "ECPU") {
                        eCore.append(
                            self.calculateFrequencies(dict: sample.delta, freqs: self.eCoreFreqs))
                    }
                    if sample.channel.starts(with: "PCPU") {
                        pCore.append(
                            self.calculateFrequencies(dict: sample.delta, freqs: self.pCoreFreqs))
                    }
                }

                let eCoresAvgFreq: Double =
                    eCore.isEmpty ? 0 : (eCore.reduce(0.0, { $0 + $1 }) / Double(eCore.count))
                let pCoresAvgFreq: Double =
                    pCore.isEmpty ? 0 : (pCore.reduce(0.0, { $0 + $1 }) / Double(pCore.count))
            ...
            let value: Double =
                ((eFreq * self.eCoreCount) + (pFreq * self.pCoreCount))
                / (self.eCoreCount + self.pCoreCount)
    ...
    private func calculateFrequencies(dict: CFDictionary, freqs: [Int32]) -> Double {
        ...
        guard
            let offset = items.firstIndex(where: {
                $0.0 != "IDLE" && $0.0 != "DOWN" && $0.0 != "OFF"
            })
        else { return 0 }
    ...
    func getResidencies(dict: CFDictionary) -> [(ns: String, f: Int64)] {
        ...
        for i in 0..<count {
            let name =
                IOReportStateGetNameForIndex(dict, i)?.takeUnretainedValue() ?? ("" as CFString)
    ...
    private func getChannels() -> CFMutableDictionary? {
        ...
        for (gname, sname) in channelNames {
            let channel = IOReportCopyChannelsInGroup(
                gname as CFString?, sname as CFString?, 0, 0, 0)
        ...
        guard let channel = CFDictionaryCreateMutableCopy(kCFAllocatorDefault, size, chan),
            let chan = channel as? [String: Any], chan["IOReportChannels"] != nil
        else {
            return nil
        }
    ...
    private func getSample() -> (samples: CFDictionary, time: TimeInterval)? {
        guard
            let sample = IOReportCreateSamples(self.subscription, self.channels, nil)?
                .takeRetainedValue()
        else {
            return nil
        }
    ...
    private func collectIOSamples(data: CFDictionary) -> [IOSample] {
        ...
        for index in 0..<itemSize {
            ...
            let subGroup =
                IOReportChannelGetSubGroup(item)?.takeUnretainedValue() ?? ("" as CFString)
            let channel =
                IOReportChannelGetChannelName(item)?.takeUnretainedValue() ?? ("" as CFString)
            let unit = IOReportChannelGetUnitLabel(item)?.takeUnretainedValue() ?? ("" as CFString)

            samples.append(
                IOSample(
                    group: group as String, subGroup: subGroup as String,
                    channel: channel as String, unit: unit as String, delta: item))
        }
