...
internal class UsageReader: Reader<RAM_Usage> {
    ...
    public override func setup() {
        ...
        self.totalSize = 0
        error(
            "host_info(): \(String(cString: mach_error_string(kerr), encoding: String.Encoding.ascii) ?? "unknown error")",
            log: self.log)
    }
    ...
    public override func read() {
        ...
        error(
            "host_statistics64(): \(String(cString: mach_error_string(result), encoding: String.Encoding.ascii) ?? "unknown error")",
            log: self.log)
    }
}
...
public class ProcessReader: Reader<[TopProcess]> {
    ...
    public override func read() {
        ...
        if self.combinedProcesses {
            ...
            task.arguments = [
                "-l", "1", "-o", "mem", "-n", "\(self.numberOfProcesses)", "-stats",
                "pid,command,mem",
            ]
        }
        ...
        for (_, processes) in processGroups {
            ...
            if let app = NSRunningApplication(
                processIdentifier: pid_t(ProcessReader.getResponsiblePid(firstProcess.pid))),
    ...
    private static let dynGetResponsiblePidFunc: UnsafeMutableRawPointer? = {
        let result = dlsym(
            UnsafeMutableRawPointer(bitPattern: -1), "responsibility_get_pid_responsible_for_pid")
        if result == nil {
    ...
    static func getResponsiblePid(_ childPid: Int) -> Int {
        ...
        let responsiblePid = unsafeBitCast(
            ProcessReader.dynGetResponsiblePidFunc, to: dynGetResponsiblePidFuncType.self)(
                CInt(childPid))
    ...
    static public func parseProcess(_ raw: String) -> TopProcess {
        ...
        {
            command = regex.stringByReplacingMatches(
                in: command, options: [], range: NSRange(location: 0, length: command.count),
                withTemplate: "")
        }
