...
public class Portal: NSStackView, Portal_p {
    ...
    init(_ module: ModuleType, list: [Clock_t]) {
        ...
        super.init(
            frame: NSRect(
                x: 0, y: 0, width: Constants.Popup.width, height: Constants.Popup.portalHeight))
        ...
        self.container.widthAnchor.constraint(equalToConstant: Constants.Popup.width).isActive =
            true

        self.addArrangedSubview(
            PortalHeader(
                name,
                titleOverride: "\(localizedString(self.name)) (\(localizedString("Local Only")))")
        )
    ...
    public func callback(_ list: [Clock_t]) {
        ...
        var views = self.container.subviews.filter { $0 is ClockView }.compactMap {
            $0 as? ClockView
        }
        ...
        let width: CGFloat = self.frame.width - self.edgeInsets.left - self.edgeInsets.right
        ...
        sorted.forEach { (c: Clock_t) in
            if let view = views.first(where: { $0.clock.id == c.id }) {
                ...
                self.container.addArrangedSubview(ClockView(width: width, clock: c))
            }
        }
