...
open class PortalWrapper: NSStackView, Portal_p {
    ...
    public init(
        _ type: ModuleType, title: String? = nil, height: CGFloat = Constants.Popup.portalHeight
    ) {
        self.name = type.stringValue
        self.header = PortalHeader(type.stringValue, titleOverride: title)
...
public class PortalHeader: NSStackView {
    ...
    public init(_ name: String, titleOverride: String? = nil) {
        self.name = name
        ...
        title.stringValue = titleOverride ?? localizedString(name)
    ...
    @objc private func openSettings() {
        ...
        NotificationCenter.default.post(
            name: .toggleSettings, object: nil, userInfo: ["module": self.name])
    }
}
