...
public struct module_c {
    ...
    init(in path: String) {
        ...
        if self.icon == nil, #available(macOS 11.0, *),
            let symbol = dict["AlternativeSymbol"] as? String
        {
            self.icon = NSImage(systemSymbolName: symbol, accessibilityDescription: nil)
        }
        ...
        if let widgetsDict = dict["Widgets"] as? NSDictionary {
            ...
            self.availableWidgets = list.sorted(by: { $0.1 < $1.1 }).map {
                (widget_t(rawValue: $0.key) ?? .unknown)
            }
        }
...
open class Module {
    ...
    public init(
        moduleType: ModuleType, popup: Popup_p? = nil, settings: Settings_v? = nil,
        portal: Portal_p? = nil, notifications: NotificationsWrapper? = nil
    ) {
        self.moduleType = moduleType
        self.portal = portal
        self.config = module_c(
            in: Bundle(for: type(of: self)).path(forResource: "config", ofType: "plist")!)
        ...
        self.enabled = Store.shared.bool(
            key: "\(self.config.name)_state", defaultValue: self.config.defaultState)
        ...
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenForMouseDownInSettings), name: .clickInSettings,
            object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenForModuleToggle), name: .toggleModule, object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenForPopupToggle), name: .togglePopup, object: nil)
        NotificationCenter.default.addObserver(
            self, selector: #selector(listenForToggleWidget), name: .toggleWidget, object: nil)
        ...
        self.popup = PopupWindow(
            title: self.config.name, module: self.moduleType, view: self.popupView,
            visibilityCallback: self.visibilityCallback)
    }
        ...
        if let name = notification.userInfo?["module"] as? String {
            ...
            if self.pauseState == true {
                ...
                NotificationCenter.default.post(
                    name: .pause, object: nil, userInfo: ["state": false])
            }
        }
    }
