...
public struct LaunchAtLogin {
    ...
    private static var isEnabledLegacy: Bool {
        get {
            guard let jobs = (LaunchAtLogin.self as DeprecationWarningWorkaround.Type).jobsDict
            else {
                return false
            }
...
extension LaunchAtLogin: DeprecationWarningWorkaround {
    ...
    static var jobsDict: [[String: AnyObject]]? {
        SMCopyAllJobDictionaries(kSMDomainUserLaunchd)?.takeRetainedValue()
            as? [[String: AnyObject]]
    }
}
...
extension NSBezierPath {
    public func addArrow(
        start: CGPoint, end: CGPoint, pointerLineLength: CGFloat, arrowAngle: CGFloat
    ) {
        self.move(to: start)
        self.line(to: end)
...
public func separatorView(
    _ title: String, origin: NSPoint = NSPoint(x: 0, y: 0), width: CGFloat = 0
) -> NSView {
    ...
    let labelView: NSTextField = TextView(
        frame: NSRect(x: 0, y: (view.frame.height - 15) / 2, width: view.frame.width, height: 15))
...
public func popupRow(_ view: NSView? = nil, title: String, value: String, multiline: Bool = false)
    -> (LabelField, ValueField, NSView)
{
    ...
    let labelView: LabelField = LabelField(
        frame: NSRect(x: 0, y: ((22 - 16) / 2) + ((lines - 1) * 16), width: labelWidth, height: 16),
        title)
    let valueView: ValueField = ValueField(
        frame: NSRect(
            x: labelWidth, y: (22 - 16) / 2, width: rowView.frame.width - labelWidth,
            height: multiline ? 16 * lines : 16), value)
...
public func portalRow(
    _ v: NSStackView, title: String, value: String = "", isSelectable: Bool = false
) -> (LabelField, ValueField, NSStackView) {
...
public func popupWithColorRow(_ view: NSView, color: NSColor, title: String, value: String) -> (
    ...
    let labelWidth = min(
        180, title.widthOfString(usingFont: .systemFont(ofSize: 13, weight: .regular)) + 5)
    let labelView: LabelField = LabelField(
        frame: NSRect(x: 18, y: (22 - 16) / 2, width: labelWidth, height: 16), title)
    let valueView: ValueField = ValueField(
        frame: NSRect(
            x: 18 + labelWidth, y: (22 - 16) / 2, width: rowView.frame.width - labelWidth - 18,
            height: 16), value)
...
public func portalWithColorRow(_ v: NSStackView, color: NSColor, title: String) -> (
    NSView, ValueField
) {
    let view: NSStackView = NSStackView()
    view.orientation = .horizontal
...
public func isNewestVersion(currentVersion: String, latestVersion: String) -> Bool {
    ...
    var current = Version(
        major: currentArray.indices.contains(0) ? (Int(currentArray[0]) ?? 0) : 0,
        minor: currentArray.indices.contains(1) ? (Int(currentArray[1]) ?? 0) : 0,
        patch: currentArray.indices.contains(2) ? (Int(currentArray[2]) ?? 0) : 0
    )
    var latest = Version(
        major: latestArray.indices.contains(0) ? (Int(latestArray[0]) ?? 0) : 0,
        minor: latestArray.indices.contains(1) ? (Int(latestArray[1]) ?? 0) : 0,
        patch: latestArray.indices.contains(2) ? (Int(latestArray[2]) ?? 0) : 0
    )
    ...
    if current.beta == nil && latest.beta == nil {
        ...
        if latest.patch > current.patch && latest.minor >= current.minor
    ...
    if current.beta != nil && latest.beta != nil {
        ...
        if latest.beta! > current.beta! && latest.patch >= current.patch
...
public func showNotification(
    title: String, subtitle: String? = nil, userInfo: [AnyHashable: Any] = [:],
...
public func fetchIOService(_ name: String) -> [NSDictionary]? {
    ...
    let result = IOServiceGetMatchingServices(
        kIOMasterPortDefault, IOServiceMatching(name), &iterator)
    if result != kIOReturnSuccess {
        print(
            "Error IOServiceGetMatchingServices(): "
                + (String(cString: mach_error_string(result), encoding: String.Encoding.ascii)
                    ?? "unknown error"))
        return nil
    }
...
internal func convertCFDataToArr(_ data: CFData, _ isM4: Bool = false) -> [Int32] {
    ...
    for chunk in chunks {
        let v =
            UInt32(chunk[0]) | UInt32(chunk[1]) << 8 | UInt32(chunk[2]) << 16 | UInt32(chunk[3])
            << 24
...
public class ColorView: NSView {
    ...
    public init(
        frame: NSRect = NSRect.zero, color: NSColor, state: Bool = false, radius: CGFloat = 2
    ) {
        self.color = color
        self.state = state
...
extension UnitTemperature {
    public static var system: UnitTemperature {
        let measureFormatter = MeasurementFormatter()
        let measurement = Measurement(value: 0, unit: UnitTemperature.celsius)
        return measureFormatter.string(from: measurement).hasSuffix("C") ? .celsius : .fahrenheit
    }

    public static var current: UnitTemperature {
        let stringUnit: String = Store.shared.string(
            key: "temperature_units", defaultValue: "system")
        var unit = UnitTemperature.system
        if stringUnit != "system" {
            if let value = TemperatureUnits.first(where: { $0.key == stringUnit }),
                let temperatureUnit = value.additional as? UnitTemperature
            {
...
public func temperature(
    _ value: Double, defaultUnit: UnitTemperature = UnitTemperature.celsius, fractionDigits: Int = 0
) -> String {
...
public func sysctlByName(_ name: String) -> Int64 {
    ...
    if sysctlbyname(name, &num, &size, nil, 0) != 0 {
        print(
            POSIXError.Code(rawValue: errno).map { POSIXError($0) } ?? CocoaError(.fileReadUnknown))
    }
...
public class SMCHelper {
    ...
    public var isInstalled: Bool {
        syncShell("ls /Library/PrivilegedHelperTools/").contains("com.4ving.Mornits.SMC.Helper")
    }
    ...
    public func checkForUpdate() {
        let helperURL = Bundle.main.bundleURL.appendingPathComponent(
            "Contents/Library/LaunchServices/com.4ving.Mornits.SMC.Helper")
        guard
        ...
        if !self.isInstalled {
            print("SMC helper is not installed, installing...")
            self.install { _ in }
            return
        }

        guard let helper = self.helper({ _ in
            print("SMC helper connection failed, reinstalling...")
    ...
    public func install(completion: @escaping (_ installed: Bool) -> Void) {
        ...
        if SMJobBless(
            kSMDomainUserLaunchd, "com.4ving.Mornits.SMC.Helper" as CFString, authRef, &error)
            == false
        {
    ...
    private func helperConnection() -> NSXPCConnection? {
        ...
        let connection = NSXPCConnection(
            machServiceName: "com.4ving.Mornits.SMC.Helper", options: .privileged)
    ...
    private func helper(_ completion: ((Bool) -> Void)?) -> HelperProtocol? {
        ...
        guard
            let service = helper.remoteObjectProxyWithErrorHandler({ error in
                print(error)
            }) as? HelperProtocol
        else {
            completion?(false)
    ...
    public func uninstall(silent: Bool = false) {
        ...
        if !silent {
            NotificationCenter.default.post(
                name: .fanHelperState, object: nil, userInfo: ["state": false])
        }
    }
}
...
internal func restoreNSStatusItemPosition(id: String) {
    let prevPosition = Store.shared.int(
        key: "NSStatusItem Restore Position \(id)", defaultValue: -1)
    if prevPosition != -1 {
...
public class AppIcon: NSView {
    ...
    public init() {
        super.init(
            frame: NSRect(x: 0, y: 3, width: AppIcon.size.width, height: AppIcon.size.height))
    }
    ...
    public override func draw(_ dirtyRect: NSRect) {
        ...
        ctx.saveGState()
        drawUnderLine(
            dirtyRect, path: downloadLine, color: NSColor.systemBlue, x: x, y: zero - offset)
        ctx.restoreGState()
        ctx.saveGState()
    ...
    private func drawLine(points: [(CGFloat, CGFloat)], color: NSColor, lineWidth: CGFloat)
        -> NSBezierPath
    {
    ...
    private func drawUnderLine(
        _ rect: NSRect, path: NSBezierPath, color: NSColor, x: CGFloat, y: CGFloat
    ) {
...
public func showAlert(
    _ message: String, _ information: String? = nil, _ style: NSAlert.Style = .informational
) {
    let alert = NSAlert()
...
public class PreferencesSection: NSStackView {
    ...
    public init(label: String = "", id: String? = nil, _ components: [NSView] = []) {
        ...
        self.container.layer?.backgroundColor =
            NSColor.quaternaryLabelColor.withAlphaComponent(0.025).cgColor
    ...
    public func setRowVisibility(_ at: Int, newState: Bool) {
        ...
        for i in self.container.subviews.indices
        where i / 2 == at && Double(i).remainder(dividingBy: 2) == 0 {
    ...
    public func setRowVisibility(_ id: String, newState: Bool) {
        guard let at = self.container.subviews.firstIndex(where: { $0.identifier?.rawValue == id })
        else { return }
    ...
    public func findRow(_ id: String) -> PreferencesRow? {
        let rows: [PreferencesRow] = self.container.subviews.filter({ $0 is PreferencesRow })
            .compactMap({ $0 as? PreferencesRow })
...
public class PreferencesRow: NSStackView {
    ...
    public init(
        _ title: String? = nil, _ description: String? = nil, id: String? = nil, component: NSView,
        help: (() -> Void)? = nil
    ) {
        ...
        self.edgeInsets = NSEdgeInsets(
            top: Constants.Settings.margin / 2, left: 0,
            bottom: (Constants.Settings.margin / 2) - 1, right: 0)
        self.spacing = 0
        if let id {
            self.identifier = NSUserInterfaceItemIdentifier(id)
        }
...
public class StepperInput: NSStackView, NSTextFieldDelegate, PreferencesSwitchWith_p {
    ...
    @objc private func onUnitChange(_ sender: NSMenuItem) {
        guard let key = sender.representedObject as? String, let units = self.units,
            let value = units.first(where: { $0.key == key })
        else { return }
...
public class PreferencesSwitch: NSStackView {
    ...
    public init(
        action: @escaping (_ sender: NSControl) -> Void, state: Bool, with: PreferencesSwitchWith_p
    ) {
        self.action = action
        self.with = with
...
public class HelpHUD: NSPanel {
    ...
    public init(
        _ text: String, origin: CGPoint = CGPoint(x: 0, y: 0),
        size: CGSize = CGSize(width: 420, height: 300)
    ) {
        self.text = text
        super.init(
    ...
    public func show() {
        if self.contentView as? WKWebView == nil {
            ...
            webView.loadHTMLString(
                "<html><body style='color: #ffffff;margin: 10px;'>\(self.text)</body></html>",
                baseURL: nil)
...
public class GPUStressTest {
    ...
    private func allocateMemory() {
        ...
        self.bufferA = self.device.makeBuffer(
            length: self.dataSize * MemoryLayout<Float>.size, options: .storageModeShared)
        self.bufferB = self.device.makeBuffer(
            length: self.dataSize * MemoryLayout<Float>.size, options: .storageModeShared)
        self.bufferC = self.device.makeBuffer(
            length: self.dataSize * MemoryLayout<Float>.size, options: .storageModeShared)
    ...
    private func test() {
        ...
        while self.isRunning {
            guard let commandBuffer = self.commandQueue.makeCommandBuffer(),
                ...
                let bufferA = self.bufferA, let bufferB = self.bufferB, let bufferC = self.bufferC
            else {
                break
            }
