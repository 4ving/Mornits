...
public class DB {
    ...
    private let queue = DispatchQueue(label: "com.4ving.db")
    ...
    init() {
        ...
        let supportPath = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("Mornits")
        let tmpPath = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("Mornits")
