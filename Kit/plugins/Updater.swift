...
public class Updater {
    ...
    public func install(path: String, completion: @escaping (_ error: String?) -> Void) {
        let pwd = Bundle.main.bundleURL.absoluteString
            ...
            .replacingOccurrences(of: "Mornits.app", with: "")
        ...
        _ = syncShell("mkdir /tmp/Mornits") // make sure that directory exist
        let res = syncShell("/usr/bin/hdiutil attach \(path) -mountpoint /tmp/Mornits -noverify -nobrowse -noautoopen") // mount the dmg
        ...
        if res.contains("is busy") { // dmg can be busy, if yes, unmount it and mount again
            ...
            _ = syncShell("/usr/bin/hdiutil detach $TMPDIR/Mornits")
            _ = syncShell("/usr/bin/hdiutil attach \(path) -mountpoint /tmp/Mornits -noverify -nobrowse -noautoopen")
        }
        
        _ = syncShell("cp -rf /tmp/Mornits/Mornits.app/Contents/Resources/Scripts/updater.sh $TMPDIR/updater.sh") // copy updater script to tmp folder
