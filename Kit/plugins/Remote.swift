...
public class Remote {
    ...
    public func login() {
        self.auth.login { url in
            ...
            debug("Open \(url) to login to Mornits Remote", log: self.log)
    ...
    public func logout() {
        ...
        debug("Logout successfully from Mornits Remote", log: self.log)
    ...
    @objc private func successLogin() {
        ...
        NotificationCenter.default.post(name: .remoteState, object: nil, userInfo: ["auth": self.isAuthorized])
        self.mqtt.connect()
        debug("Login successfully on Mornits Remote", log: self.log)
    }
