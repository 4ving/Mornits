...
internal func scaleValue(
    scale: Scale = .linear, value: Double, maxValue: Double, zeroValue: Double, maxHeight: CGFloat,
    limit: Double
) -> CGFloat {
    var value = value
...
private func drawToolTip(
    _ frame: NSRect, _ point: CGPoint, _ size: CGSize, value: String, subtitle: String? = nil
) {
    ...
    position.x = max(
        frame.origin.x, min(position.x, frame.origin.x + frame.size.width - size.width))
    position.y = max(
        frame.origin.y, min(position.y, frame.origin.y + frame.size.height - textHeight - 2))
    ...
    let box = NSBezierPath(
        roundedRect: NSRect(
            x: position.x - 3, y: position.y - 2, width: size.width, height: textHeight + 2),
        xRadius: 2, yRadius: 2)
    ...
    if let subtitle {
        ...
        attributes[NSAttributedString.Key.foregroundColor] =
            (isDarkMode ? NSColor.white : NSColor.textColor).withAlphaComponent(0.7)
...
public class LineChartView: NSView {
    ...
    private var queue: DispatchQueue = DispatchQueue(
        label: "com.4ving.Mornits.charts.line", attributes: .concurrent)
    ...
    public init(
        frame: NSRect, num: Int, suffix: String = "%", color: NSColor = .controlAccentColor,
        scale: Scale = .none, fixedScale: Double = 1, zeroValue: Double = 0.01
    ) {
    ...
    public override func draw(_ dirtyRect: NSRect) {
        ...
        for (i, v) in points.enumerated() {
            ...
            var y = scaleValue(
                scale: scale, value: v.value, maxValue: maxValue, zeroValue: zeroValue,
                maxHeight: height, limit: fixedScale)
            if flipY {
                y = height - y
            }
        ...
        for linePoints in lines {
            if linePoints.count == 1 {
                path = NSBezierPath(
                    ovalIn: CGRect(
                        x: linePoints[0].x - offset, y: linePoints[0].y - offset, width: 1,
                        height: 1))
                lineColor.set()
                path.stroke()
                gradientColor.set()
        ...
        if minMax {
            let stringAttributes = [
                ...
                NSAttributedString.Key.foregroundColor: isDarkMode
                    ? NSColor.white : NSColor.textColor,
            ...
            if let value = flatList.max() {
                str =
                    toolTipFunc != nil
                    ? toolTipFunc!(DoubleValue(value))
                    : "\(Int(value.rounded(toPlaces: 2) * 100))\(suffix)"
            }
            let textWidth = str.widthOfString(
                usingFont: stringAttributes[NSAttributedString.Key.font] as! NSFont)
        ...
        if value {
            ...
            var str: String = ""
            if let v = originalPoints.last {
                let val = v?.value ?? 0
                str =
                    toolTipFunc != nil
                    ? toolTipFunc!(DoubleValue(val))
                    : "\(Int(val.rounded(toPlaces: 2) * 100))\(suffix)"
            }
            let textWidth = str.widthOfString(
                usingFont: stringAttributes[NSAttributedString.Key.font] as! NSFont)
            let y = flipY ? 1 : height - 9
            let rect = CGRect(
                x: self.frame.width - textWidth - 2, y: y, width: textWidth, height: 8)
            NSAttributedString.init(string: str, attributes: stringAttributes).draw(with: rect)
        }
        ...
        if isTooltipEnabled, let p = self.cursor, !list.isEmpty {
                ...
                let under = underPoints.max(by: { $0.point.x < $1.point.x })
            {
                ...
                let strValue =
                    roundedValue >= 1 ? "\(Int(roundedValue))\(suffix)" : "\(roundedValue)\(suffix)"
                let value = toolTipFunc != nil ? toolTipFunc!(nearest.value) : strValue
                drawToolTip(
                    self.bounds, CGPoint(x: nearest.point.x + 4, y: nearest.point.y + 4),
                    CGSize(width: 78, height: height), value: value, subtitle: date)
            }
        }
    }
    ...
    public func reinit(_ num: Int = 60) {
        ...
        if num < self.points.count {
            ...
            self.points.replaceSubrange(
                Range(uncheckedBounds: (lower: num - origin.count, upper: num)), with: origin)
        }
    }
...
public class NetworkChartView: NSView {
    ...
    public init(
        frame: NSRect, num: Int, minMax: Bool = true, value: Bool = true,
        reversedOrder: Bool = false,
        outColor: NSColor = .systemRed, inColor: NSColor = .systemBlue, scale: Scale = .none,
        fixedScale: Double = 1
    ) {
        ...
        let topFrame = NSRect(
            x: frame.origin.x, y: safeHeight / 2, width: frame.width, height: safeHeight / 2)
        let bottomFrame = NSRect(
            x: frame.origin.x, y: 0, width: frame.width, height: safeHeight / 2)
        let inFrame = self.reversedOrder ? topFrame : bottomFrame
        let outFrame = self.reversedOrder ? bottomFrame : topFrame
        self.inChart = LineChartView(
            frame: inFrame, num: num, color: inColor, scale: scale, fixedScale: fixedScale,
            zeroValue: 256.0)
        self.outChart = LineChartView(
            frame: outFrame, num: num, color: outColor, scale: scale, fixedScale: fixedScale,
            zeroValue: 256.0)
        ...
        self.inChart.value = value
        self.outChart.value = value
...
public class PieChartView: NSView {
    ...
    private var queue: DispatchQueue = DispatchQueue(label: "com.4ving.Mornits.charts.pie")

    public init(
        frame: NSRect, segments: [circle_segment], filled: Bool = false, drawValue: Bool = false
    ) {
        self.filled = filled
    ...
    public override func draw(_ rect: CGRect) {
        ...
        if totalAmount < 1 {
            segments.append(
                circle_segment(
                    value: Double(1 - totalAmount),
                    color: nonActiveSegmentColor.withAlphaComponent(0.5)))
        }
        ...
        for segment in segments.reversed() {
            ...
            context.addArc(
                center: centerPoint, radius: radius, startAngle: previousAngle,
        ...
        if let value = value, drawValue {
            ...
            let rect = CGRect(
                x: (self.frame.width - width) / 2, y: (self.frame.height - 11) / 2, width: width,
                height: 12)
...
public class HalfCircleGraphView: NSView {
    ...
    private var queue: DispatchQueue = DispatchQueue(label: "com.4ving.Mornits.charts.halfcircle")
    ...
    public override func draw(_ rect: CGRect) {
        ...
        if value < 1 {
            segments.append(
                circle_segment(
                    value: Double(1 - value), color: NSColor.lightGray.withAlphaComponent(0.5)))
        }
        ...
        if let text = text {
            ...
            let rect = CGRect(
                x: ((self.frame.width - width) / 2) - 0.5, y: (self.frame.height - 6) / 2,
...
internal class TachometerGraphView: NSView {
    ...
    private var segments: [circle_segment]
    private var queue: DispatchQueue = DispatchQueue(label: "com.4ving.Mornits.charts.tachometer")
    ...
    public override func draw(_ rect: CGRect) {
        ...
        if totalAmount < 1 {
            segments.append(
                circle_segment(
                    value: Double(1 - totalAmount), color: NSColor.lightGray.withAlphaComponent(0.5)
                ))
        }
...
public class BarChartView: NSView {
    ...
    private var queue: DispatchQueue = DispatchQueue(label: "com.4ving.Mornits.charts.bar")
    ...
    public override func draw(_ dirtyRect: NSRect) {
        ...
        let partitionSize: CGSize = CGSize(
            width: (self.frame.width - (count * spacing)) / count, height: self.frame.height)
        let blockSize = CGSize(
            width: partitionSize.width - (spacing * 2),
            height: ((partitionSize.height - spacing - 1) / CGFloat(blocks)) - 1)
        ...
        for i in 0..<values.count {
            let partition = NSBezierPath(
                roundedRect: NSRect(
                    x: x, y: 0, width: partitionSize.width, height: partitionSize.height),
                xRadius: 3, yRadius: 3
            )
            ...
            if dirtyRect.height < 30 && h != 0 {
                let block = NSBezierPath(
                    roundedRect: NSRect(
                        x: x + spacing, y: 1, width: partitionSize.width - (spacing * 2), height: h),
                    xRadius: 1, yRadius: 1
                )
                color.setFill()
                block.fill()
                ...
                for b in 0..<blocks {
                    let block = NSBezierPath(
                        roundedRect: NSRect(
                            x: x + spacing, y: y, width: blockSize.width, height: blockSize.height),
                        xRadius: 1, yRadius: 1
                    )
                    (activeBlockNum <= b
                        ? NSColor.controlBackgroundColor.withAlphaComponent(0.4) : color).setFill()
                    block.fill()
                    block.close()
        ...
        if let p = self.cursor {
            ...
            if let block = matchingBlock {
                ...
                drawToolTip(
                    self.frame, CGPoint(x: tooltipX, y: tooltipY),
                    CGSize(width: width, height: min(partitionSize.height, self.frame.height)),
                    value: value)
            }
        }
    }
...
public class GridChartView: NSView {
    ...
    private var queue: DispatchQueue = DispatchQueue(label: "com.4ving.Mornits.charts.grid")
    ...
    public override func draw(_ dirtyRect: NSRect) {
        ...
        let size: CGSize = CGSize(
            ...
            height: (self.frame.height - ((CGFloat(grid.columns) - 1) * spacing))
